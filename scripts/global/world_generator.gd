...
const TILE_COUNT = 50000
const LANDMASS_COUNT = 1
...
func generate_world(planet: Planet) -> void:
	...
	print("Generating world map - Starting time: %s" % DataBus.TIME_SINCE_START)
	var world_map := WorldMap.new(_build_world_map(TILE_COUNT))
	DataBus.ACTIVE_WORLD = world_map
	print("Generated world map - Time since start: %s" % DataBus.TIME_SINCE_START)
	print("World map size: %s" % world_map.get_num_tiles())
	print("World map rows: %s" % world_map.get_num_rows())
	print("World map cols: %s" % world_map.get_num_cols())

	map_gen_complete.emit()

	var seed_tiles = _initialize_land_seeds()

	seed_tiles = [world_map.get_tile(100, 250)]

	generation_step_complete.emit()

	print("Growing landmasses - Starting time: %s" % DataBus.TIME_SINCE_START)
	var landmass_dict = _grow_landmasses(seed_tiles)
	DataBus.ACTIVE_LANDMASSES = landmass_dict
	print("Finished growing landmasses - Time since start: %s" % DataBus.TIME_SINCE_START)
	print("Landmasses: %s" % landmass_dict.keys())

	# _create_islands(world_map)
	# print("Islands created")

func _build_world_map(total_tiles: int) -> Array:
	var num_rows := int(sqrt(float(total_tiles) / 2.0))
	var num_cols := num_rows * 2
	var world_map := []
...
func _grow_landmasses(seed_tiles: Array) -> Dictionary:
	...
	for seed_tile in seed_tiles:
		print("Seed tile coords: ", seed_tile.coords())
